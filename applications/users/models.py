...
class RegistroCreditosModel(models.Model):
    usuarios = models.ForeignKey(User,on_delete=models.CASCADE,related_name='usuarios_registro',blank=True,null=True)
    producto = models.ForeignKey(Prestamo,on_delete=models.CASCADE,related_name='producto_registro',blank=True,null=True)
    fecha_credito = models.DateField(null=True,blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.identificador_unico:
            ...
            fecha_formato = self.fecha_credito.strftime('%d%m%Y')
            # Obtener el último identificador único en la base de datos
            ultimo_identificador = RegistroCreditosModel.objects.latest('id').identificador_unico if RegistroCreditosModel.objects.exists() else 0
            ...
            nuevo_numero = (ultimo_numero + 5) % 100  # Limitar a dos dígitos (0-99)
